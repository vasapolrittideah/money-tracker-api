...
func main() {
	...
	serviceID := authServiceCfg.Name + "__" + uuid.New().String()
	if err := consulRegistry.Register(serviceID, authServiceCfg.Name, authServiceCfg.AddressForConsul); err != nil {
	...
	authUsecase := usecase.NewAuthUsecase(identityRepo, sessionRepo, userRepo, jwtAuth, authServiceCfg)
